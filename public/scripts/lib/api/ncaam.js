...
function normalizeParticipant(raw, fallbackTeamId) {
...
function isThreePoint(stat, qualifier, shotType) {
...
function resolveTeamId(stat, fallbackTeamId) {
...
function normalizeStatistic(raw, fallbackTeamId, participants) {
    ...
    const detectPlayerByRole = (role) => {
        const participant = participants.find(part => part.id !== null && part.id !== undefined && typeof part.role === "string" && part.role.toLowerCase().includes(role));
...
function hasPositiveResult(stat) {
...
function normalizeEvent(event, fallbackSequence) {
    ...
    const participants = Array.isArray(event.participants)
        ...
        : [];
    const stats = Array.isArray(event.statistics)
        ? event.statistics
...
export async function getGamePlayByPlay(gameId) {
    ...
    const response = await ncaam(`/games/${normalizedId}/playbyplay`);
