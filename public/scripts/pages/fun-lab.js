...
function measureContainerSize(element) {
...
function readBarRadius(element) {
    const doc = element.ownerDocument ?? document;
    const view = doc.defaultView;
    if (!view) {
        return 8;
...
function renderCatsDogsChart(matchups, chartContainer, colorByMatchup, handle) {
    ...
    handle.mount(() => {
        ...
        const buildSegments = (accessor, programAccessor, barKey) => {
            let start = 0;
            const segments = [];
        ...
        segmentGroups.each(function (bar) {
            ...
            rects
                ...
                .attr("data-matchup", (segment) => segment.slug)
                .attr("role", "button")
                .attr("tabindex", 0)
                .attr("aria-pressed", "false")
                .attr("x", (segment) => pixelAlign(x(segment.start)))
                .attr("y", 0)
                .attr("width", (segment) => {
        ...
        segmentsSelection.on("keydown", (event, segment) => {
        ...
        rows
            ...
            .attr("x", (bar) => pixelAlign(Math.max(0, x(bar.total) - 12)))
            .attr("y", barHeight / 2)
            .attr("dy", "0.35em")
            .attr("text-anchor", "end")
...
function renderCatsDogsLeaderboard(list, matchups, colorByMatchup) {
    list.innerHTML = "";
    if (matchups.length === 0) {
