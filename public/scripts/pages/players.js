...
if (!(treeElement instanceof HTMLElement))
    throw new Error("Missing roster tree container");
...
function groupTeamsByConference(conferences, teams) {
    const map = new Map();
...
function createIndependentSummary(id) {
    return { id, name: "Independent", short_name: "Independent" };
}
function renderConferences(container, groups) {
    container.innerHTML = "";
    for (const group of groups) {
        container.append(createConferenceDetails(group));
    }
}
function createConferenceDetails(group) {
...
async function loadTeamRoster(details, team, force = false) {
    ...
    const container = details.querySelector("[data-team-roster]");
    if (!container) {
    ...
    try {
        ...
        replaceChildren(container, createRetryStatus("Unable to load the active roster.", () => {
...
function renderStatus(target, message, kind) {
    replaceChildren(target, createStatus(message, kind));
}
function renderRetry(target, message, action) {
    replaceChildren(target, createRetryStatus(message, action));
}
function createStatus(message, kind) {
    const status = document.createElement("div");
    status.className = "roster-status";
    if (kind === "error")
        status.classList.add("roster-status--error");
    if (kind === "empty")
        status.classList.add("roster-status--empty");
    const p = document.createElement("p");
    p.textContent = message;
    status.append(p);
    return status;
}
function createRetryStatus(message, action) {
