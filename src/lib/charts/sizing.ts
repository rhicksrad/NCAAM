...
const visibilityRegistry = new WeakMap<Element, VisibilityState>();

function resolveWindow(root: Element): (Window & typeof globalThis) | null {
 ...
 */
export function setupChartSizing(root: HTMLElement, ratio = 0.6): () => void {
 ...
 */
export function registerWhenVisible(root: Element, mount: () => void): void {
 ...
 */
export function cleanupVisibility(root: Element): void {
