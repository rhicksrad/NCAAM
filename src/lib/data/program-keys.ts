...
const PROGRAM_LABEL_TRANSFORMS: Array<(value: string) => string> = [
...
export function buildProgramLabelKeys(label: string | null | undefined): string[] {
  if (!label) {
    return [];
  }
...
function deriveSchoolName(team: Team): string | undefined {
...
const TEAM_VALUE_TRANSFORMS: Array<(value: string) => string> = [
  value => value,
  value => value.replace(/\u2013/g, "-"),
...
export function buildTeamKeys(team: Team): string[] {
