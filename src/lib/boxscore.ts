import type { PlayByPlayEvent, PlayByPlayTeam } from "./api/ncaam.js";
...
function cleanupName(name: string | null | undefined): string | null {
  if (!name) {
    return null;
  }
...
function deriveTeamFromPlayTeam(team: PlayByPlayTeam | null, fallbackId: number | null): Team | null {
  if (!team) {
    return null;
  }
...
function createTeam(team: Team | null, teamId: number | null): MutableTeam {
  return {
    teamId,
    team: team ?? null,
    players: new Map(),
...
function ensureTeam(
  ...
  playTeam: PlayByPlayTeam | null,
): MutableTeam {
  const key = Number.isFinite(teamId) ? (teamId as number) : null;
...
function isLikelyTeamLabel(name: string | null, team: Team | null, playTeam: PlayByPlayTeam | null): boolean {
  if (!name) {
    return false;
  }
...
function applyStat(stat: NumericStat, value: number, player: MutablePlayer | null, team: MutableTeam) {
...
function updateReboundTotals(player: MutablePlayer | null, team: MutableTeam) {
  if (player) {
...
function extractShotShooter(description: string): string | null {
  const match = description.match(/^(?<name>.+?)\s+(?:made|missed)\b/i);
  return cleanupName(match?.groups?.name);
}

function extractAssistName(description: string): string | null {
  const match = description.match(/Assisted by ([^.]+)/i);
  return cleanupName(match?.[1]);
}

function extractRebounder(description: string): string | null {
  const match = description.match(/^(?<name>.+?)\s+(?:Offensive|Defensive) Rebound/i);
  return cleanupName(match?.groups?.name);
}

function extractStealer(description: string): string | null {
  const match = description.match(/^(?<name>.+?)\s+Steal/i);
  return cleanupName(match?.groups?.name);
}

function extractBlocker(description: string): string | null {
  const match = description.match(/^(?<name>.+?)\s+Block/i);
  return cleanupName(match?.groups?.name);
}

function extractFouler(description: string): string | null {
  const match = description.match(/Foul on ([^.]+)/i);
  return cleanupName(match?.[1]);
}

function extractTurnover(description: string): string | null {
  const match = description.match(/^(?<name>.+?)\s+Turnover/i);
  return cleanupName(match?.groups?.name);
}

function resolveShotValue(event: PlayByPlayEvent, fallback: number): number {
...
function recordFieldGoal(event: PlayByPlayEvent, team: MutableTeam) {
...
function recordFreeThrow(event: PlayByPlayEvent, team: MutableTeam) {
  const description = normalizeDescription(event.description);
  const shooter = ensurePlayer(team, extractShotShooter(description), event.team ?? null);
...
function recordRebound(event: PlayByPlayEvent, team: MutableTeam, offensive: boolean) {
  const description = normalizeDescription(event.description);
...
function recordTurnover(event: PlayByPlayEvent, team: MutableTeam) {
  const description = normalizeDescription(event.description);
  const culprit = ensurePlayer(team, extractTurnover(description), event.team ?? null);
  applyStat("tov", 1, culprit, team);
}

function recordSteal(event: PlayByPlayEvent, team: MutableTeam) {
  const description = normalizeDescription(event.description);
...
function recordBlock(event: PlayByPlayEvent, team: MutableTeam) {
  const description = normalizeDescription(event.description);
...
function recordFoul(event: PlayByPlayEvent, team: MutableTeam) {
  const description = normalizeDescription(event.description);
...
export function buildBoxScoreFromPlayByPlay({ game, events }: BuildBoxScoreOptions): GameBoxScore {
  ...
  for (const event of events) {
    ...
    const withTeam = (handler: (team: MutableTeam) => void) => {
