import { NCAAM, type ActivePlayer, type Conference, type Team } from "../lib/sdk/ncaam.js";
...
type StatusVariant = "loading" | "error" | "empty" | "hint" | "info";
...
const statusTarget = app.querySelector<HTMLElement>("[data-status]");
if (!statusTarget) {
  throw new Error("Players page layout failed to initialize");
}
const listTarget = app.querySelector<HTMLElement>("[data-conference-list]");
if (!listTarget) {
  throw new Error("Players page layout failed to initialize");
}
const statusEl: HTMLElement = statusTarget;
const listEl: HTMLElement = listTarget;

statusEl.setAttribute("aria-live", "polite");
listEl.setAttribute("hidden", "");

const conferenceStates = new Map<number, ConferenceState>();
...
function renderConferences(conferences: readonly Conference[]) {
...
function createConferenceState(conference: Conference): ConferenceState {
...
async function ensureConferenceTeams(state: ConferenceState): Promise<void> {
...
function renderConferenceState(state: ConferenceState) {
...
function createTeamState(team: Team, conference: ConferenceState): TeamState {
  const details = document.createElement("details");
...
async function loadTeamRoster(state: TeamState, force = false): Promise<void> {
  if (state.loading) {
    return;
  }
...
function renderTeamState(state: TeamState) {
...
function buildPlayerCard(player: ActivePlayer): HTMLElement {
...
function appendMeta(meta: HTMLDListElement, label: string, value: string | null) {
  if (!value) {
    return;
  }
...
async function fetchConferenceTeams(conferenceId: number): Promise<Team[]> {
  const perPage = 100;
  let page = 1;
  const teams: Team[] = [];
...
async function fetchTeamRoster(teamId: number): Promise<ActivePlayer[]> {
  const perPage = 100;
...
function findTeamLogo(team: Team): string | null {
...
function showPageStatus(variant: StatusVariant, message: string, options: { retry?: boolean } = {}) {
...
function createStatusElement(message: string, variant: StatusVariant): HTMLDivElement {
