...
type LeaderboardMinimumRequirementType = "games" | "freeThrowAttempts";

type LeaderboardMinimumRequirement = {
  type: LeaderboardMinimumRequirementType;
  value: number;
};

type StackedLeaderboardEntry = {
  leader: PlayerLeaderboardEntry;
  breakdown: Record<string, number>;
  total: number;
};

type StackedLeaderboardItemState = {
  entry: StackedLeaderboardEntry;
  element: HTMLLIElement;
  valueEl: HTMLSpanElement;
  segmentElements: Map<string, HTMLSpanElement>;
};

type LeaderboardStackedSegment = {
  id: string;
  label: string;
  stat: keyof PlayerStatsSeason;
  color: string;
};

type LeaderboardStackedView = {
  id: string;
  label: string;
  segmentIds: string[];
  useTotal?: boolean;
...
type LeaderboardStackedConfig = {
  segments: LeaderboardStackedSegment[];
  views: LeaderboardStackedView[];
  defaultView: string;
  totalStat?: keyof PlayerStatsSeason;
};

type LeaderboardMetricPresentation = {
  kicker: string;
  title: string;
  description: string;
  accentColor?: string;
  stacked?: LeaderboardStackedConfig;
};
...
const LEADERBOARD_DEFAULT_MINIMUM_REQUIREMENT: LeaderboardMinimumRequirement = {
  type: "games",
  value: 10,
};

const LEADERBOARD_MINIMUM_REQUIREMENTS: Partial<
  Record<LeaderboardMetricId, LeaderboardMinimumRequirement>
> = {
...
const LEADERBOARD_PRESENTATION: Record<LeaderboardMetricId, LeaderboardMetricPresentation> = {
  mp: {
    kicker: "Iron five",
...
const LEADERBOARD_PERCENT_METRICS = new Set<LeaderboardMetricId>(["fgPct", "fg3Pct", "ftPct"]);

const LEADERBOARD_AXIS_LABELS: Partial<Record<LeaderboardMetricId, string>> = {
  mp: "Minutes per game",
...
type LeaderboardChartDatum = BarDatum & {
  leader: PlayerLeaderboardEntry;
};

type LeaderboardChartOptions = {
...
const normaliseBasePath = (pathname: string): string => {
...
let assetBase: string | null | undefined;

const resolveAssetBase = (): string | null => {
  try {
  ...
  if (typeof document !== "undefined") {
    try {
      const baseElement = document.querySelector<HTMLBaseElement>("base[href]");
...
const getAssetBase = (): string | null => {
...
function renderPlayerCard(player: Player): HTMLElement {
  ...
  if (jersey !== "—") detailParts.push(jersey);
  const position = player.position?.trim();
  if (position) detailParts.push(position);
  const height = player.height?.trim();
  if (height) detailParts.push(height);
  const weight = player.weight?.trim();
  if (weight) detailParts.push(weight);
...
async function renderPlayerLeaderboards(
...
function getLeaderboardMinimumRequirement(
  metricId: LeaderboardMetricId,
): LeaderboardMinimumRequirement {
...
function formatLeaderboardMinimumNote(requirement: LeaderboardMinimumRequirement): string {
...
function formatLeaderboardMinimumEmptyState(requirement: LeaderboardMinimumRequirement): string {
  switch (requirement.type) {
    case "freeThrowAttempts":
...
function leaderboardEntryMeetsMinimumRequirement(
  entry: PlayerLeaderboardEntry,
  requirement: LeaderboardMinimumRequirement,
): boolean {
...
async function createLeaderboardCard(
  ...
  if (presentation?.stacked) {
    const stackedContent = await createStackedLeaderboardContent(
      metricId,
      leaders,
      doc.season,
...
function mountLeaderboardChart(
  container: HTMLElement,
  leaders: PlayerLeaderboardEntry[],
  ...
  let surface = container.querySelector<HTMLElement>('[data-chart-surface="true"]');
  if (!surface) {
  ...
  const ensureTooltip = (): TooltipHandle => {
    if (!tooltip) {
  ...
  const render = () => {
    ...
    const height = Math.max(
      320,
      margin.top + margin.bottom + leaders.length * 28,
    );
    ...
    const data: LeaderboardChartDatum[] = leaders.map(leader => ({
      x: leader.name,
    ...
    const bars = renderBars(plot.append("g").node() as SVGGElement, data, scales, {
      ...
      baseline: 0,
      innerHeight: ih,
    }) as Selection<SVGRectElement, LeaderboardChartDatum, SVGGElement, unknown>;
    ...
    const axisGroup = plot.append("g").node() as SVGGElement;
    ...
    const showTooltip = (target: Element, datum: LeaderboardChartDatum) => {
    ...
    const moveTooltip = (target: Element) => {
      const rect = target.getBoundingClientRect();
      const parentRect = container.getBoundingClientRect();
      const x = rect.left - parentRect.left + rect.width / 2;
      const y = rect.top - parentRect.top - 12;
    ...
    bars
      ...
      .on("focus", (event: FocusEvent, datum: LeaderboardChartDatum) => {
        const target = event.currentTarget as Element | null;
        if (target) {
          showTooltip(target, datum);
        }
...
function createLeaderboardChartTheme(accentColor: string): ChartTheme {
  if (!accentColor) {
    return defaultTheme;
  }
  return {
...
function usesPercentScale(metricId: LeaderboardMetricId): boolean {
...
function formatLeaderboardTooltip(
  metricId: LeaderboardMetricId,
  leader: PlayerLeaderboardEntry,
): string {
...
function getLeaderboardAxisLabel(
  metricId: LeaderboardMetricId,
  metric: PlayerLeaderboardMetric,
): string {
  return (
    LEADERBOARD_AXIS_LABELS[metricId] ??
    metric.shortLabel ??
    metric.label ??
...
async function createStackedLeaderboardContent(
  metricId: LeaderboardMetricId,
  leaders: PlayerLeaderboardEntry[],
  seasonLabel: string | undefined,
  config: LeaderboardStackedConfig,
  ...
  const processed = (
    await Promise.all(
      leaders.map(async leader => {
        ...
        const segmentSum = config.segments.reduce(
          (sum, segment) => sum + (breakdown[segment.id] ?? 0),
          0,
        );
        const totalRaw = config.totalStat ? season[config.totalStat] : null;
        const totalCandidate =
          typeof totalRaw === "number" && Number.isFinite(totalRaw) ? totalRaw : segmentSum;
        ...
        return {
          ...
          breakdown,
          total,
        } satisfies StackedLeaderboardEntry;
      }),
    )
  ).filter((entry): entry is StackedLeaderboardEntry => Boolean(entry));
  ...
  const items: StackedLeaderboardItemState[] = processed.map((entry, index) => {
    const item = document.createElement("li");
    item.className = "player-leaderboard__item";
    item.dataset.rank = String(index + 1);
    ...
    const bar = document.createElement("div");
    bar.className = "player-leaderboard__bar";
    const segmentElements = new Map<string, HTMLSpanElement>();
  ...
  const initialView =
    config.views.find(view => view.id === config.defaultView) ?? config.views[0];
  if (!initialView) {
    return container;
  }
  ...
  function updateView(viewId: string): void {
    ...
    const maxValue = computations.reduce(
      (max, current) => Math.max(max, current.baseValue),
      0,
    );
    ...
    items.forEach((itemState, index) => {
      ...
      config.segments.forEach(segment => {
        ...
        const isActive = Boolean(
          segmentData && view.segmentIds.includes(segment.id) && segmentData.value > 0,
        );
        ...
        segmentEl.classList.toggle(
          "player-leaderboard__bar-segment--hidden",
          !view.segmentIds.includes(segment.id),
        );
...
function computeStackedView(
  entry: StackedLeaderboardEntry,
  view: LeaderboardStackedView,
): { baseValue: number; segments: Array<{ id: string; value: number }> } {
...
function findSeasonForLeaderboard(
  doc: PlayerStatsDocument | null,
  seasonLabel: string | undefined,
