...
function Elo() {
  const K = 20, map = new Map<number,number>();
  const get = (id:number)=>map.get(id) ?? 1500;
  const set = (id:number,v:number)=>map.set(id,v);
  const ex = (a:number,b:number)=>1/(1+10**((b-a)/400));
  return {
    rate(a:number,b:number,sa:number){
      const Ra=get(a), Rb=get(b), Ea=ex(Ra,Rb);
      set(a, Ra + K*(sa-Ea)); set(b, Rb + K*((1-sa)-(1-Ea)));
    },
    map
  };
}
