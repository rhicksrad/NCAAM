...
import type { GameBoxScore, PlayerBoxScoreLine, TeamBoxScore } from "../../lib/boxscore.js";
...
function formatName(player: PlayerBoxScoreLine): string {
...
function formatPair(made: number, attempts: number): string {
  return `${made ?? 0}-${attempts ?? 0}`;
}

function renderPlayerRow(player: PlayerBoxScoreLine): string {
...
function renderTotalsRow(label: string, totals: TeamBoxScore["totals"]): string {
...
function renderPlayersSection(players: PlayerBoxScoreLine[]): string {
  return players.map(renderPlayerRow).join("");
}

function renderBenchSection(bench: PlayerBoxScoreLine[]): string {
  if (bench.length === 0) {
    return "";
  }
...
function renderTeamTable(teamBox: TeamBoxScore, teamLabel: string): string {
...
export function renderGameBoxScore({ game, boxScore, isLoading, error }: GameBoxScoreProps): string {
  if (isLoading) {
