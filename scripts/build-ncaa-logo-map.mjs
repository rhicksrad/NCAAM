...
const archivePath = path.join(ROOT, 'public/FBS-Logo-Library-main.zip');
const logosDir = path.join(ROOT, 'public/assets/logos/ncaa');
const markerPath = path.join(logosDir, '.source.json');
const zipPrefix = 'FBS-Logo-Library-main/CFB Logos/';
...
async function ensureNcaALogos() {
  const stat = await fs.stat(archivePath).catch(error => {
    if (error && error.code === 'ENOENT') {
      throw new Error(`NCAA logo archive missing at ${archivePath}`);
    }
    throw error;
  });

  const signature = JSON.stringify({ size: stat.size, mtimeMs: stat.mtimeMs });
  const existingSignature = await fs.readFile(markerPath, 'utf8').catch(() => null);
...
async function build() {
  ...
  const files = (await fs.readdir(logosDir)).filter(file => file.toLowerCase().endsWith('.png'));
